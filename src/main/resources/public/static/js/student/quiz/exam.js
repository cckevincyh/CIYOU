...
$(window).bind('beforeunload',function(){	
			...
			$.ajax({
	            ...
	            data: {
	                answer: dataStr,
	                quizId: $.trim($("#test_quizId").val()),
					sid:$.trim($("#test_studentId").val())
	            },
	            success: function (data) {
					editCookie($.trim($("#test_studentId").val()) + "-" + $.trim($("#test_quizId").val()), 0, 0);
...
$(function() {
			...
			if(!hasSetCookie()){//如果没有这个cookie就新增一个
				addCookie($.trim($("#test_studentId").val()) + "-" + $.trim($("#test_quizId").val()), _expiresHours, _expiresHours);	//cookieName：考生的id+考试科目,cookieValue:考试时间，expiresHours：cookie过期时间
			} 
			//开始倒计时
		...
		function hasSetCookie(){
			...
			for (var i = 0; i < arrCookie.length; i++) {
				...
				if (arr[0] == $.trim($("#test_studentId").val()) + "-" + $.trim($("#test_quizId").val())) {
					return true;
				}
			};
			return false;
		...
		function settime(remainTime) {
			var _countdown = parseInt(getCookieValue($.trim($("#test_studentId").val()) + "-" + $.trim($("#test_quizId").val()))) / 1000;	//得到倒数秒
...
//				window.location.href = "student.jsp";
				...
				_countdown--;
				editCookie($.trim($("#test_studentId").val()) + "-" + $.trim($("#test_quizId").val()), _countdown * 1000, _countdown * 1000);
			}
			//每1000毫秒执行一次
			setTimeout(function() {
				settime(remainTime);
